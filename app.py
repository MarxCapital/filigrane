...
def draw_diagonal_watermark(canvas_obj, page_w, page_h, text, subtitle=None):
    """Dessine un filigrane diagonal (coin bas-gauche -> coin haut-droit),
    centre, semi-transparent, quel que soit le texte. Si `subtitle` est
    fourni (ex : "Realisee et concue par ..."), il est dessine juste en
    dessous, dans le prolongement de la meme diagonale, dans une taille
    plus petite."""
    if not text:
        return
    ...
    if subtitle:
        sub_font_size = max(6.0, final_font_size * 0.26)
        canvas_obj.setFont("Helvetica-Bold", sub_font_size)
        offset_y = -final_font_size * 0.35 - sub_font_size * 1.6
        canvas_obj.drawCentredString(0, offset_y, subtitle)
...
def render_fiche_pages(blocks, font_size=9.5, leading_factor=1.18,
    ...
    subtitle_text = f"Réalisée et conçue par {watermark_text}" if watermark_text else None
    ...
    def _on_page(c, d):
        page_counter["n"] += 1
        draw_diagonal_watermark(c, page_size[0], page_size[1], watermark_text, subtitle=subtitle_text)
...
def build_watermark_overlay(page_w: float, page_h: float, new_name: str, subtitle_text: str) -> bytes:
    """Construit un calque PDF (1 page) contenant uniquement le filigrane
    diagonal, aux dimensions reelles de la page."""
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=(page_w, page_h))
    draw_diagonal_watermark(c, page_w, page_h, new_name, subtitle=subtitle_text)
    c.save()
    buf.seek(0)
    return buf.read()
...
def add_diagonal_watermark_overlay(pdf_bytes: bytes, new_name: str, skip_pages=None) -> bytes:
    """Ajoute le filigrane diagonal "maison" (avec la mention
    "Realisee et concue par ...") sur toutes les pages, sauf celles listees
    dans `skip_pages` (indices 0-based) : ce sont les pages deja
    ...
    subtitle_text = f"Réalisée et conçue par {new_name}"
    reader = PdfReader(io.BytesIO(pdf_bytes))
    writer = PdfWriter()
    ...
    for idx, page in enumerate(reader.pages):
        ...
        if key not in overlay_cache:
            overlay_bytes = build_watermark_overlay(w, h, new_name, subtitle_text)
